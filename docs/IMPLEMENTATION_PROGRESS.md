...
- latest focused runtime regression lane: `node --test generated/tests/item-system.test.js generated/tests/app-engine-account-economy.test.js generated/tests/app-engine-world-nodes.test.js generated/tests/run-progression-simulator.test.js generated/tests/app-engine-progression.test.js`
- latest deterministic balance lanes: `npm run sim:progression-class-sweep -- --policy aggressive --through-act 5 --probe-runs 0 --seeds 4` and `npm run sim:progression-class-sweep -- --policy balanced,control,bulwark --through-act 2 --probe-runs 0 --seeds 4`
- latest rune or runeword progression spot-check: `npm run sim:progression -- --class barbarian,sorceress --policy aggressive --through-act 3 --probe-runs 0`
...
## Balance and Simulation Workflow

Use these commands as the standard balance loop:

- `npm run sim:skill-audit`
  - audits card-value outliers by cost, tier, and role
- `npm run sim:balance -- --class barbarian,sorceress --scenario mainline_conservative,mainline_rewarded --set act5_endgame --runs 8`
  - checks endgame encounter pacing, win rate, and party-vs-enemy power on a fixed boss or elite set
- `npm run sim:progression -- --class barbarian,sorceress --policy aggressive --through-act 5`
  - runs a deterministic campaign sim with act checkpoints, power score, top cards, weapon, armor, and active runewords
- `npm run sim:progression-class-sweep -- --policy aggressive --through-act 5 --probe-runs 0 --seeds 4`
  - checks optimized clear-rate bands across the roster
- `npm run sim:progression-class-sweep -- --policy balanced,control,bulwark --through-act 2 --probe-runs 0 --seeds 4`
  - checks early-failure pressure on weaker policies
- `npm run sim:power-curve -- --class barbarian --policy aggressive --through-act 5`
  - compares checkpoint probes to the target boss or elite or battle ratio bands

Current workflow rule:

- rerun the relevant sweep after major combat, loot, or class-card changes
- treat the balance snapshot in `artifacts/balance/latest.md` as current guidance, not a permanent design spec
- freeze balance unless the new deterministic sample shows a clear outlier
...
- quest-driven runeforge rewards that can finish early runeword projects through the normal reward seam
...
- boss-guaranteed rune drops, missing-rune loot weighting for unfinished runeword projects, and deeper vendor rune routing
- unique-only bonus seams like `+1` hand size, wired through combat, UI, and simulator scoring
- combat stat bonuses from loadout

Remaining:
- final late-run loot tuning
- more authored late-run unique and item breadth
...
8. keep the deterministic balance snapshot honest by rerunning the class-policy sweeps after major combat, loot, or class-card changes, then freeze balance unless the new sample shows a clear outlier
