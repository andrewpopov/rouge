...
test("safe-zone services can heal, refill, and change mercenary contracts without losing map progress", () => {
  const { content, combatEngine, appEngine, runFactory, browserWindow, seedBundle } = createHarness();
  const state = appEngine.createAppState({
    content,
    seedBundle,
    combatEngine,
  ...
  const blackPit = state.run.acts[0]?.zones?.find((zone) => zone.title === "Den of Evil");
  assert.ok(blackPit);
