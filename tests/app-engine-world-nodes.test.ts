...
function resolveActOneToCovenant(state, appEngine, runFactory) {
...
test("detour lane outcomes can retune the next branch battle beyond the recovery baseline", () => {
  ...
  assert.equal(result.ok, true);
  assert.equal(state.phase, appEngine.PHASES.ENCOUNTER);
  assert.equal(state.run.activeEncounterId, "act_1_branch_detour");
  assert.equal(state.combat.encounter.name, "Monastery Detour Line");
  ...
  assert.equal(reward.grants.gold, 24);
  assert.equal(reward.grants.xp, 14);
  assert.equal(reward.grants.potions, 1);
  assert.ok(reward.lines.some((line) => line.includes("Late-route payoff: Abbey Convoy Dividend.")));
  assert.ok(reward.lines.some((line) => line.includes("hidden abbey convoy")));
});
...
test("escalation lane outcomes can retune the next branch miniboss beyond the accord baseline", () => {
  ...
  assert.equal(result.ok, true);
  assert.equal(state.phase, appEngine.PHASES.ENCOUNTER);
  assert.equal(state.run.activeEncounterId, "act_1_miniboss_escalation");
  assert.equal(state.combat.encounter.name, "Burial Grounds Escalation Host");
  ...
  assert.equal(reward.grants.gold, 32);
  assert.equal(reward.grants.xp, 20);
  assert.equal(reward.grants.potions, 2);
  assert.ok(reward.lines.some((line) => line.includes("Late-route payoff: Catacomb Surge Dividend.")));
  assert.ok(reward.lines.some((line) => line.includes("catacomb surge")));
});
...
test("detour and escalation outcomes can retune the act boss beyond the covenant baseline", () => {
  ...
  assert.equal(result.ok, true);
  assert.equal(state.phase, appEngine.PHASES.ENCOUNTER);
  assert.equal(state.run.activeEncounterId, "act_1_boss_aftermath");
  assert.equal(state.combat.encounter.name, "Catacomb Aftermath");
  assert.ok(state.combat.encounter.modifiers.some((modifier) => modifier.kind === "boss_screen"));
  assert.ok(state.combat.encounter.modifiers.some((modifier) => modifier.kind === "sniper_nest"));
  assert.ok(state.combat.encounter.modifiers.some((modifier) => modifier.kind === "linebreaker_charge"));
  ...
  assert.equal(reward.grants.gold, 54);
  assert.equal(reward.grants.xp, 34);
  assert.equal(reward.grants.potions, 2);
  assert.ok(reward.lines.some((line) => line.includes("Late-route payoff: Catacomb Aftermath Dividend.")));
  assert.ok(reward.lines.some((line) => line.includes("Detour, escalation, and covenant now settle together")));
});
