...
(() => {
  ...
  function ensureHistoryEntry(entry: unknown, content: GameContent | null = null) {
    ...
    return sanitizeHistoryPlanningEntry({
      ...
      unlockedClassSkills: toNumber(source.unlockedClassSkills, 0),
      loadoutTier: toNumber(source.loadoutTier, 0),
      loadoutSockets: toNumber(source.loadoutSockets, 0),
      carriedEquipmentCount: toNumber(source.carriedEquipmentCount, 0),
      carriedRuneCount: toNumber(source.carriedRuneCount, 0),
      stashEntryCount: toNumber(source.stashEntryCount, 0),
      stashEquipmentCount: toNumber(source.stashEquipmentCount, 0),
      stashRuneCount: toNumber(source.stashRuneCount, 0),
